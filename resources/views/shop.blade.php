...
<section class="product store-product">
    <div class="container">
        ...
        <div class="d-flex justify-content-between align-items-center gap-3 mb-4">
            <div class="position-relative">
                <input type="text" placeholder="{{ __('Search products')}}" class="search-input bg-slate-100 rounded-pill border-0 text-gray-900 font-inter-regular pe-4">
                <svg class="search-icon position-absolute top-50 translate-middle-y left-0 ms-3" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <select name="sortBy" id="sortBy" class="form-control w-25 ms-auto d-none sortBy">
                <option value=""> --- Sort By  --- </option>
                <option value="DESC">Sort By: newest</option>
            </select>
            <div class="filter-menu d-flex gap-3">
                <button type="button" class="border-0 bg-slate-100 rounded-pill d-flex align-items-center justify-content-center productOrderFilter" data-order-filter="" id="productOrderFilter">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M15 18H3V16H15V18ZM21 13H3V11H21V13ZM15 8H3V6H15V8Z" fill="#1C2531"></path>
                    </svg>
                </button>
                <button type="button" class="border-0 bg-slate-100 rounded-pill d-flex align-items-center justify-content-center productFilterShowBtn" data-category-filter="" id="productFilterShowBtn">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M22.5 6H19.425C19.05 4.275 17.55 3 15.75 3C13.95 3 12.45 4.275 12.075 6H1.5V7.5H12.075C12.45 9.225 13.95 10.5 15.75 10.5C17.55 10.5 19.05 9.225 19.425 7.5H22.5V6ZM15.75 9C14.475 9 13.5 8.025 13.5 6.75C13.5 5.475 14.475 4.5 15.75 4.5C17.025 4.5 18 5.475 18 6.75C18 8.025 17.025 9 15.75 9ZM1.5 18H4.575C4.95 19.725 6.45 21 8.25 21C10.05 21 11.55 19.725 11.925 18H22.5V16.5H11.925C11.55 14.775 10.05 13.5 8.25 13.5C6.45 13.5 4.95 14.775 4.575 16.5H1.5V18ZM8.25 15C9.525 15 10.5 15.975 10.5 17.25C10.5 18.525 9.525 19.5 8.25 19.5C6.975 19.5 6 18.525 6 17.25C6 15.975 6.975 15 8.25 15Z" fill="#1C2531"></path>
                    </svg>
                </button>
        ...
        @if (isset($filters))
            @if (!$filters->isEmpty())
            <div class="row bg-slate-100 rounded-2 border-0 text-gray-900 font-inter-regular py-2 my-3 filters-row d-none">
                    ...
                    @foreach ($filter->options as $option)
                        <li>
                            <div class="form-check">
                                <input class="filter_options form-check-input productCategoryFilter" type="checkbox" value="{{$option->id}}" data-option-value="{{$filter->id}}" name="filter_options[]" id="{{$option->id}}">
        ...
        <div class="row" id="filteredProductList">
        </div>
...
<script>
    $(document).ready(function() {
        ...
        let categoryName = @json($categoryName);
        var sortValues = '';
        var search = '';
        var pageNo = 1;
        ...
        function productFilters() {
            ...
            $('input[name="filter_options[]"]:checked').each(function() {
                ...
                if (!optionsValues.includes($(this).data('option-value'))) {
                    optionsValues.push($(this).data('option-value'));
                }
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: {
                    'option_id' : optionsValues,
                    'filter_id' : filterValues,
                    'filter' : filter,
                ...
                success: function (response) {
                    if(response.success) {
                        ...
                        if(response.html) {
                            $('#filteredProductList').html(response.html);
                        } else {
                            $('#filteredProductList').empty();
                        }

                        if(response.pagination) {
                            $('#ProductPagination').html(response.pagination);
                        }
        ...
        $(document).on('change', '.productCategoryFilter', function() {
            productFilters();            
        });
        ...
        $(document).on('click', '.productOrderFilter', function() {
            let sortBy = $(document).find('.sortBy');
            if(sortBy.hasClass('d-none')) {
        ...
        $(document).on('click', '.productFilterShowBtn', function() {
            let FiltersRow = $(document).find('.filters-row');
            if(FiltersRow.hasClass('d-none')) {
        ...
        $(document).on('click', '.page-change[data-page]', function(e) {
            e.preventDefault();
